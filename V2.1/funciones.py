...
def buscar_objetos(busqueda):
    try:
        connection = connectionBD()
        cursor = connection.cursor(dictionary=True)
        query = """
        SELECT IdProducto, NombreProducto, CantidadProducto, DescripcionProducto, 
               TipoProducto, ObservacionesProducto
        FROM productosgenerales 
        WHERE NombreProducto LIKE %s OR DescripcionProducto LIKE %s
        """
        search_term = f"%{busqueda}%"
        cursor.execute(query, (search_term, search_term))
        resultados = cursor.fetchall()
        print(f"Búsqueda: '{busqueda}', Resultados encontrados: {len(resultados)}")
