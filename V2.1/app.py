...
app.secret_key = 'tu_clave_secreta_aqui'  # Asegúrate de que esto esté configurado
...
def buscar_route():
    ...
    print(f"Término de búsqueda recibido: '{busqueda}'")
    resultados = buscar_objetos(busqueda)
    print(f"Número de resultados: {len(resultados)}")
    if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
        print("Solicitud AJAX detectada")
        return render_template('resultadoBusqueda.html', miData=resultados, busqueda=busqueda)
    else:
        print("Solicitud no-AJAX detectada")
        return render_template('resultadoBusqueda.html', miData=resultados, busqueda=busqueda)
...
def logout():
    try:
        ...
        return redirect(url_for('login_route'))  # Cambiado de 'loginusu' a 'login_route'
    except Exception as e:
        print(f"Error durante el cierre de sesión: {str(e)}")
        flash('Ocurrió un error al cerrar sesión', 'error')
        return redirect(url_for('home'))  # Cambiado de 'index' a 'home' o el nombre correcto de tu ruta principal
